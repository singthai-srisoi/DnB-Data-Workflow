...
def get_data_db():
    ...
    try:
        query = session.query(Customer).all()
        if not query:
            return None
...
def pre_import_data_db(file: bytes, sheet_name: str = 'Maintain Customer') -> tuple[List[Customer], pl.DataFrame, pl.DataFrame]:
    ...
    return (customer_list, pl.DataFrame(customer_upload), df)
...
def import_from_sqlacc():
    ...
    try:
        ...
        UserService.insert_unimported(session, com_server, model='AR_CUSTOMER')
...
if import_source == "Upload File":
    st.subheader("Upload a file to import customer data.")
    ...
    if st.session_state.pre_upload_data:
        ...
        customer_objects = st.session_state.pre_upload_data[0]
        ...
        if not preview_df.is_empty():
            ...
            st.text(f"Total records to upload: {len(customer_objects)}")
            if st.button("Import Data"):
                import_data_db(customer_objects)
                st.session_state.pre_upload_data = None
                st.session_state.sheets_name = None
                st.session_state.df = get_data_db()
...
# --- SQLAcc Direct Import Mode ---
    ...
    #     import_from_sqlacc()
    #     st.session_state.df = get_data_db()
    st.subheader("Preview Unimported Customer Codes from SQLAcc")
    ...
    if st.button("Check Unimported Codes"):
        ...
        try:
            com_server = get_comserver()
            unimported = UserService.compare(session, com_server, model='AR_CUSTOMER')
